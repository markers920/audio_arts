...
random.seed(11011922) #datetime.now()) #1987
...
def build_music():
    ...
    tempo = int(100) #192/2     # In BPM
    beats_per_measure = 4
    selected_chord_progression = chords.chord_progression_CGaF #chord_progression_eadgbe #.chord_progression_aFCG #chord_progression_CFGF #chord_progression_CGaF
    number_of_cycles_1 = 3
    number_of_cycles_2 = 1
    ...
    #set instruments ----------------------------------------------------
    percussion_track, percussion_channel, percussion_volume = 0, 0, 100
    midi_file.addProgramChange(percussion_track, percussion_channel, 0, midi_instrument_numbers.woodblock) #acoustic_grand_piano) #woodblock)
    
    base_track, base_channel, base_volume = 1, 1, 90
    midi_file.addProgramChange(base_track, base_channel, 0, 
        midi_instrument_numbers.pad_7_halo) #percussive_organ) #pan_flute) #acoustic_grand_piano) #pan_flute) #tubular_bells) #acoustic_grand_piano)
    
    top_1_track, top_1_channel, top_1_volume = 2, 2, 90
    midi_file.addProgramChange(top_1_track, top_1_channel, 0, 
        midi_instrument_numbers.percussive_organ) #rock_organ) #pan_flute) #acoustic_grand_piano) #acoustic_grand_piano) #string_ensemble_2)
    
    top_2_track, top_2_channel, top_2_volume = 3, 3, 110
    midi_file.addProgramChange(top_2_track, top_2_channel, 0, 
        midi_instrument_numbers.celesta) #acoustic_grand_piano) #string_ensemble_1)
    
    top_3_track, top_3_channel, top_3_volume = 3, 3, 110
    midi_file.addProgramChange(top_3_track, top_3_channel, 0, 
        midi_instrument_numbers.pan_flute) #acoustic_grand_piano) #tubular_bells)
    ...
    for cycle_index_1 in range(number_of_cycles_1):
        ...
        percussion_chord = [notes.NOTE_D4] #, notes.NOTE_E4, notes.NOTE_F4]
